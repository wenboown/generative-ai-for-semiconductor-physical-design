...
font_size = 100  # Font size for annotations
annotations = [(chr(65 + i % 26) + str(i // 26 + 1)) for i in range(26 * 13)]  # Generating annotations A1 - Z13

def create_fiducial_marks(cell, center, gap, width, labels):
    for idx, label in enumerate(labels):
        x_offset = (idx % 13) * gap - (6 * gap)  # 0 to 12, centered around origin
        y_offset = (idx // 13) * gap - (6 * gap)  # 0 to 12, centered around origin
